...
import { Upload, Save, Trash2, Home, Image as ImageIcon, Shield, LogIn, Edit, Plus } from 'lucide-react';
...
}> = ({ title, image, onFileSelect, onRemove, aspectRatio = 'aspect-video', icon }) => {
  ...
  return (
    <div>
      ...
      <div
        ...
        className={`relative group cursor-pointer ${aspectRatio} bg-slate-800/50 border-2 border-dashed border-slate-700 rounded-xl flex items-center justify-center text-slate-500 hover:border-teal-500 hover:text-teal-400 transition-all`}
      >
        {image ? (
            <>
...
const LinkInput: React.FC<{ label: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; }> = ({ label, value, onChange }) => (
    <div>
        ...
        <input
            ...
            value={value}
            onChange={onChange}
            className="w-full h-10 px-3 bg-slate-800 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
        />
    </div>
);

const EditorAdminPanel: React.FC = () => {
  ...
  useEffect(() => {
    // Load images
    setHeroImage(localStorage.getItem('editor_hero_image'));
    ...
    setDownloadLink(localStorage.getItem('editor_download_link') || '');
    setSourceLink(localStorage.getItem('editor_source_link') || '');
  ...
  const handleSave = () => {
    // Images
    if (heroImage) localStorage.setItem('editor_hero_image', heroImage); else localStorage.removeItem('editor_hero_image');
    ...
    if (downloadLink) localStorage.setItem('editor_download_link', downloadLink); else localStorage.removeItem('editor_download_link');
    if (sourceLink) localStorage.setItem('editor_source_link', sourceLink); else localStorage.removeItem('editor_source_link');
    
    // Updates
    localStorage.setItem('editor_updates', JSON.stringify(updates));
  ...
  return (
    <div className="w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl shadow-teal-900/20 relative">
      <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
        <h1 className="text-3xl font-bold text-white">پنل مدیریت - Editor</h1>
        <a href="/" className="flex items-center gap-2 text-slate-400 hover:text-teal-400 transition-colors">
          <Home size={20} />
      ...
      <div>
        <h2 className="text-2xl font-bold text-white mb-2">تصویر اصلی صفحه Editor</h2>
        <p className="text-slate-400 mb-8">
          تصویر اصلی که در بالای صفحه Editor نمایش داده می‌شود را آپلود کنید.
        </p>
        <div className="max-w-xl">
          <ImageUploader
            title="تصویر اصلی (Hero)"
            image={heroImage}
            onFileSelect={(file) => handleFileSelect(file, setHeroImage)}
            onRemove={() => handleRemoveImage('editor_hero_image', setHeroImage)}
          />
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-slate-800 pt-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <LinkInput label="لینک دانلود Editor" value={downloadLink} onChange={(e) => setDownloadLink(e.target.value)} />
            <LinkInput label="لینک سورس کد Editor" value={sourceLink} onChange={(e) => setSourceLink(e.target.value)} />
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-slate-800 pt-6">
        <div className="flex justify-between items-center mb-4">
            <div>
                ...
                <p className="text-slate-400 mt-2">آخرین تغییرات و آپدیت‌های مربوط به نسخه Editor را مدیریت کنید.</p>
            </div>
            <button onClick={handleAddUpdate} className="flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition-colors">
                <Plus size={16}/>
                افزودن آپدیت
            </button>
        </div>
                    ...
                    {editingUpdate?.id === update.id ? (
                         <div className="space-y-3">
                            <input
                                ...
                                className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white font-bold focus:outline-none focus:ring-2 focus:ring-teal-500"
                            />
                            <textarea
                                value={editingUpdate.changes}
                                onChange={(e) => handleUpdateChange(update.id, 'changes', e.target.value)}
                                rows={4}
                                className="w-full p-3 bg-slate-700 border border-slate-600 rounded-lg text-white font-mono focus:outline-none focus:ring-2 focus:ring-teal-500"
                            />
      ...
      <div className="grid md:grid-cols-2 gap-8 mt-8 border-t border-slate-800 pt-6">
          <div>
            ...
            <p className="text-slate-400 mb-8">
              لوگویی که در تب مرورگر نمایش داده می‌شود را تغییر دهید. (تنظیم سراسری)
            </p>
          ...
          <div>
            ...
            <p className="text-slate-400 mb-8">
                لوگویی که در بالای صفحه کنار عنوان نمایش داده می‌شود. (تنظیم سراسری)
            </p>
            <div className="max-w-xs">
              <ImageUploader
                title="آپلود لوگو"
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-slate-800">
        <button
          onClick={handleSave}
          className="flex-1 inline-flex items-center justify-center h-12 px-6 font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-colors shadow-lg shadow-teal-500/30"
        >
...
const LoginPage: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
  ...
  return (
    <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl shadow-teal-900/20">
      ...
      <form onSubmit={handleSubmit}>
        <input
          ...
          disabled={isLocked}
          className="w-full h-12 px-4 bg-slate-800 border border-slate-700 rounded-lg text-white text-center text-lg tracking-widest focus:outline-none focus:ring-2 focus:ring-teal-500 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={isLocked}
          className="mt-4 w-full inline-flex items-center justify-center h-12 px-6 font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-colors shadow-lg shadow-teal-500/30 disabled:bg-slate-700 disabled:shadow-none disabled:cursor-not-allowed"
        >
...
const EditorAdminPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4">
      {isAuthenticated ? <EditorAdminPanel /> : <LoginPage onLogin={() => setIsAuthenticated(true)} />}
    </div>
  );
};

export default EditorAdminPage;